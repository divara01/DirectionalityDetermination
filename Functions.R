...
#Use - Determine Direction of Information Flow From and To Omental From and To Magic Module -- Situations #5-8
...
####Information_Flow_Btwn_Omental_Permuted_MM####
#Use - Determine Direction of Information Flow From and To Omental From and To Permuted Magic Module DO NOT USE
Information_Flow_Btwn_Omental_Permuted_MM <- function(temp_df, numSamples, num_neighbors, mode_magic, mode_neighbors){
  for (sample_index in 1:numSamples){ 
    #1. Identify permuted MM
    permuted_MM <- sample(V(g_whole_omental)$name, size_MM, replace = FALSE)
    permuted_pos <- match(permuted_MM, V(g_whole_omental)$name)
    neighbors <- neighborhood(g_whole_omental, order = 1, nodes = permuted_pos, mode = "in")
    neighbors <- unique(unlist(neighbors))
    length(neighbors)
    
    permuted_RO_network_genes <- V(g_whole_omental)$name[which(!(V(g_whole_omental)$name %in% permuted_MM))]
    
    permuted_RO_network_pos <- match(permuted_RO_network_genes, V(g_whole_omental)$name)
    
    g_permuted_RO <- induced.subgraph(g_whole_omental, permuted_RO_network_pos, impl = "auto")
    
    length(V(g_RO)$name) #4847
    core_magic_pos_on_omental <- match(permuted_MM, V(g_whole_omental)$name)
    core_magic_pos_on_omental <- core_magic_pos_on_omental[!is.na(core_magic_pos_on_omental)]
    ...
    #4. Remove Magic Module genes from RO neighbors list
    RO_neighbors <- unlist_magic_neighbors[which(!(unlist_magic_neighbors %in% permuted_MM))] # This is my list of Bs
    
    #5a. Get neighbors of these RO genes (this will be your subnetwork for each B out to num_neighbors)
    RO_pos_on_omental <- match(RO_neighbors, V(g_permuted_RO)$name)
    
    RO_neighbors_list <- neighborhood(g_permuted_RO, order = num_neighbors, nodes = RO_pos_on_omental, mode = mode_neighbors)
    
    #6a. Convert RO_neighbors_list to Entrez ID
    for (list_index in 1:length(RO_neighbors_list)){
      RO_neighbors_list[[list_index]] <- V(g_permuted_RO)$name[RO_neighbors_list[[list_index]]]
    }
    rm(list_index)
    
    for(i in 1:length(RO_neighbors_list)){
      RO_neighbors_list[[i]] <- RO_neighbors_list[[i]][which(is.na(match(RO_neighbors_list[[i]], permuted_MM)))]
    }
    rm(i)
    ...
    for (i in 1:length(RO_neighbors_list)){
      ...
      if(num_genes_SN != 0){
        ...
        num_magic_from_OSN <-  length(which(unlist_neighbors_x_list_from_OSN %in% permuted_MM))
        num_RO_from_OSN <- length(which(!(unlist_neighbors_x_list_from_OSN %in% permuted_MM))) 
        ratio_from_OSN = ifelse(num_RO_from_OSN == 0, ifelse(num_magic_from_OSN == 0, NaN, Inf), num_magic_from_OSN/num_RO_from_OSN)
        
        num_RO_from_ISN <- length(which(!(unlist_neighbors_x_list_from_ISN %in% permuted_MM))) 
        ...
        num_magic_to_OSN <-  length(which(unlist_neighbors_x_list_to_OSN %in% permuted_MM))
        num_RO_to_OSN <- length(which(!(unlist_neighbors_x_list_to_OSN %in% permuted_MM))) 
        ratio_to_OSN = ifelse(num_RO_to_OSN == 0, ifelse(num_magic_to_OSN == 0, NaN, Inf), num_magic_to_OSN/num_RO_to_OSN)
        
        num_RO_to_ISN <- length(which(!(unlist_neighbors_x_list_to_ISN %in% permuted_MM))) 
      }
      ...
      row.names(temp_df)[nrow(temp_df)] <- paste("S", sample_index, RO_neighbors_list[[i]][1], "d", num_neighbors, sep= "_")
    }
    
    #Sort by highest connections to MM
    #   temp_df <- temp_df[order(temp_df[,2],  decreasing = TRUE),]
  }      
   return(temp_df)
}
...
####GenerateOmentalSubnetworks_InfoFlowOmentalMM Function####
#Use - Determine Direction of Information Flow From and To Omental From and To Magic Module (THIS FUNCTION ONLY WORKS FOR SITUATION 1-4 AND NOT PERMUTED)-- DO NOT USE
GenerateOmentalSubnetworks_InfoFlowOmentalMM <- function(num_neighbors, mode_magic, mode_neighbors){
  #1. Identify first degree neighbors of magic module genes
  core_magic_pos_on_omental <- match(magic_module_genes$EntrezID, V(g_whole_omental)$name)
  core_magic_pos_on_omental <- core_magic_pos_on_omental[!is.na(core_magic_pos_on_omental)]
  ...
  #4. Remove Magic Module genes from RO neighbors list
  RO_neighbors <- unlist_magic_neighbors[which(!(unlist_magic_neighbors %in% magic_module_genes$EntrezID))] #369 This is my list of Bs
  
  #5a. Get neighbors of these RO genes (this will be your subnetwork for each B out to num_neighbors)
  RO_pos_on_omental <- match(RO_neighbors, V(g_RO)$name)
  
  RO_neighbors_list <- neighborhood(g_RO, order = num_neighbors, nodes = RO_pos_on_omental, mode = mode_neighbors)
  
  #6a. Convert RO_neighbors_list to Entrez ID
  for (list_index in 1:length(RO_neighbors_list)){
    RO_neighbors_list[[list_index]] <- V(g_RO)$name[RO_neighbors_list[[list_index]]]
  }
  rm(list_index)
  
  for(i in 1:length(RO_neighbors_list)){
    RO_neighbors_list[[i]] <- RO_neighbors_list[[i]][which(is.na(match(RO_neighbors_list[[i]], magic_module_genes$EntrezID)))]
  }
  rm(i)
  ...
  for (i in 1:length(RO_neighbors_list)){
    ...
    if(num_genes_SN != 0){
      ...
      num_RO_from_ISN <- length(which(!(unlist_neighbors_x_list_from_ISN %in% magic_module_genes$EntrezID))) 
      ...
      num_RO_to_ISN <- length(which(!(unlist_neighbors_x_list_to_ISN %in% magic_module_genes$EntrezID))) 
    }
  ...
  temp_df <- temp_df[-1,]
  #temp_list <- list(temp_df, RO_neighbors_list)
  #Sort by highest connections to MM
  #   temp_df <- temp_df[order(temp_df[,4],  decreasing = TRUE),]
